...
import com.yalantis.ucrop.view.photoview.PhotoView;
...
public class TransformImageView extends ImageView {
    ...
    public final float[] mMatrixValues = new float[MATRIX_VALUES_COUNT];
    ...
    public RationChanged onRationChanged;

    Uri mInputUri, mOutputUri;
    ...
    /**
     * post_content : They say a person needs just three things to be truly happy in this world: someone to love, something to do, and something to hope for
     */
    ...
    public interface TransformImageListener {
        ...
        void newRotateListner(float currentAngle);

        void onScale(float currentScale);

        void onDrawNewScales();
    }

    public interface RationChanged {
    ...
    public void setOnratioChanged(RationChanged onRationChanged) {
        this.onRationChanged = onRationChanged;
    }
    ...
    public void setImageBitmap(Bitmap bitmap, boolean redundent) {
    ...
    public interface returnBitmap {
        public void returnBitmap(Bitmap bitmap , double factorW , double factorH);
    }

    public Uri getmInputUri() {
        return mInputUri;
    }

    public void setmInputUri(Uri mInputUri) {
    ...
    public void setmOutputUri(Uri mOutputUri) {
    ...
    public void setImageMatrix(Matrix matrix, boolean red) {
    ...
    public void postScale(float deltaScale, float px, float py) {
        if (deltaScale != 0) {
            ...
            Log.e("TranspostScale", "deltaScale == 0");
        }
    }
    ...
    public void postScaleMine(Matrix matrix, float deltaScale, float px, float py) {
        ...
        if (deltaScale != 0) {
            ...
            setImageMatrix(matrix, false);

            if (mTransformImageListener != null) {
                mTransformImageListener.onScale(getMatrixScale(matrix));
            }
    ...
    public void postRotate(float deltaAngle, float px, float py) {
        if (deltaAngle != 0) {
            ...
            setImageMatrix(mCurrentImageMatrix, false);
            if (mTransformImageListener != null) {
                mTransformImageListener.newRotateListner(getMatrixAngle(mCurrentImageMatrix));
//                mTransformImageListener.onRotate(getMatrixAngle(mCurrentImageMatrix));
    ...
    public void postRotateMine(float deltaAngle, float px, float py) {
        if (deltaAngle != 0) {
            mCurrentImageMatrix.postRotate(deltaAngle, px, py);
            setImageMatrix(mCurrentImageMatrix, false);
            if (mTransformImageListener != null) {
...
//        }
    ...
    public void onFinishRaioChange(){
        if (mTransformImageListener != null) {
           onRationChanged.onRationChanged();
        }
    }
...
//            mTransformImageListener.onLoadComplete();
    ...
    public void printMatrix(@NonNull String logPrefix, @NonNull Matrix matrix) {
        ...
        Log.e(TAG, logPrefix + ": matrix: { x: " + x + ", y: " + y + ", scale: " + rScale + ", angle: " + rAngle + " }");
    }
    ...
    public void updateCurrentImagePointsMine(Matrix matrix, float[] mCurrentImageCorners, float[] mInitialImageCorners,
                                             float[] mCurrentImageCenter, float[] mInitialImageCenter) {
        try {
    ...
    public Matrix getmCurrentImageMatrix() {
