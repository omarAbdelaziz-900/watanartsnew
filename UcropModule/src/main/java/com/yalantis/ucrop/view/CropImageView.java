...
public class CropImageView extends TransformImageView {
    ...
    public static final float SOURCE_IMAGE_ASPECT_RATIO = -5f;
    ...
    getNewXYpositions getNewXYpositions;
    ...
    public void setmCropRect(RectF crop) {
        mCropRect = crop;
    }


    public void setCurrentPositions(RectF mCropRect) {
...
//        Log.e("Dimensions" , "x is "+cropOffsetX +" -  y is "+cropOffsetY +"   width is "+mCroppedImageWidth + "     height is "+mCroppedImageHeight );
    ...
    public void getImageState(boolean largeDimensions) {
        ...
        if (getNewXYpositions != null)
            getNewXYpositions.TransmitNewPositions(cropOffsetX, cropOffsetY, mCroppedImageWidth, mCroppedImageHeight);
    }
    ...
    public void cropAndSaveImage(@NonNull Bitmap.CompressFormat compressFormat, int compressQuality,
        ...
        final CropParameters cropParameters = new CropParameters(
                mMaxResultImageSizeX, 0, 0, mMaxResultImageSizeY,
                compressFormat, compressQuality,
                getImageInputPath(), getImageOutputPath(), getExifInfo());

//        new BitmapCropTask(getViewBitmap(), getmInputUri(), imageState, cropParameters, cropCallback).execute();
    }
    ...
    public void setGetNewXYpositions(getNewXYpositions getNewXYpositions) {
        this.getNewXYpositions = getNewXYpositions;
    }
...
//        getImageState(false);
    ...
    public void setTargetAspectRatio(float targetAspectRatio, int mainWidth, int mainHeight) {
...
//        }
    ...
    public void zoomInImageMine(Matrix matrix, float scale, float centerX, float centerY) {
        if (scale <= getMaxScale()) {
            postScaleMine(matrix, scale / getCurrentScale(), centerX, centerY);
        }
    }
...
//        Log.e("deltaScale", "" + deltaScale);
//        Log.e("getCurrentScale", "" + getCurrentScale());
//        Log.e("getMaxScale", "" + getMaxScale());
//        Log.e("getMinScale", "" + getMinScale());
//        Log.e("getCurrentSc*deltaScale", "" + getCurrentScale() * deltaScale);
        ...
        if (deltaScale > 1 && getCurrentScale() * deltaScale <= getMaxScale()) {
            ...
            Log.e("Else", "Else postScale");
        }
    }

    public void postScaleMine(Matrix matrix, float deltaScale, float px, float py) {
...
//        Log.e("getCurrentSc*deltaScale", "" + getCurrentScale() * deltaScale);
        ...
        if (deltaScale > 1 && getCurrentScale() * deltaScale <= getMaxScale()) {
            super.postScaleMine(matrix, deltaScale, px, py);
        } else if (deltaScale < 1 && getCurrentScale() * deltaScale >= getMinScale()) {
            super.postScaleMine(matrix, deltaScale, px, py);
        } /*else {
            Log.e("Else", "Else postScale");
    ...
    public void postRotateMine(float deltaAngle) {
        postRotateMine(deltaAngle, mCropRect.centerX(), mCropRect.centerY());
    }
    ...
    public void zoomImageToPositionMine(Matrix matrix, float scale, float centerX, float centerY) {
        if (scale > getMaxScale()) {
            scale = getMaxScale();
        }

        final float oldScale = 0;
        final float deltaScale = scale - oldScale;
        post(mZoomImageToPositionRunnable = new ZoomImageToPositionMine(CropImageView.this,
                0, oldScale, deltaScale, centerX, centerY, matrix));
    }

    public void zoomImageToPositionMine2(float scale, float centerX, float centerY, long durationMs) {
        if (scale > getMaxScale()) {
            scale = getMaxScale();
        }
        ...
        final float oldScale = 0;
        final float deltaScale = scale - oldScale;
        post(mZoomImageToPositionRunnable = new ZoomImageToPositionMine2(CropImageView.this,
                durationMs, oldScale, deltaScale, centerX, centerY));
    }
...
//TODO:change values
        ...
        setImageMatrix(mCurrentImageMatrix, false);
    }
     ...
     */
    private class WrapCropBoundsRunnable implements Runnable {
    ...
    private static class ZoomImageToPosition implements Runnable {
        ...
        public void run() {
            ...
            if (currentMs < mDurationMs) {
//                cropImageView.zoomInImage(mDeltaScale, mDestX, mDestY);
    ...
    private static class ZoomImageToPositionMine implements Runnable {
        ...
        public ZoomImageToPositionMine(CropImageView cropImageView,
                                       long durationMs,
                                       float oldScale, float deltaScale,
                                       float destX, float destY,
                                       Matrix matrix) {
        ...
        public void run() {
            ...
            if (currentMs < mDurationMs) {
//                cropImageView.zoomInImage(mDeltaScale, mDestX, mDestY);
                cropImageView.zoomInImageMine(mMatrix, mOldScale + newScale, mDestX, mDestY);
                cropImageView.post(this);
            } else {
                cropImageView.setImageToWrapCropBounds();
            }
    ...
    private static class ZoomImageToPositionMine2 implements Runnable {
        ...
        public ZoomImageToPositionMine2(CropImageView cropImageView,
                                   long durationMs,
                                   float oldScale, float deltaScale,
                                   float destX, float destY) {
...
//                cropImageView.zoomInImage(mOldScale + newScale, mDestX, mDestY);
    ...
    public interface getNewXYpositions {
        void TransmitNewPositions(float x, float y, int width, int height);
    }
}
