...
public abstract class SelectableAdapter<VH extends RecyclerView.ViewHolder>
    ...
    public List<Photo> getCurrentPhotos()  {
        List<Photo> photoList = photoDirectories.get(currentDirectoryIndex).getPhotos();

        //must delete with Iterator , otherwise cause exception
        for (Iterator<Photo> it = photoList.iterator(); it.hasNext(); ) {
            Photo photo = it.next();
            /*if (deletedPhotos.contains(photo.getPath())) {
                it.remove();
            }*/
            for (String path : deletedPhotos){
