...
/**
 * Created by mohamed.arafa on 3/6/2018.
 */
...
public class ThumbnailsAdapter extends RecyclerView.Adapter<ThumbnailsAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        if (selectedIndex == position) {
            holder.filterName.setTextColor(ContextCompat.getColor(mContext, com.yalantis.ucrop.R.color.ucrop_filter_label_selected));
        } else {
            holder.filterName.setTextColor(ContextCompat.getColor(mContext, com.yalantis.ucrop.R.color.ucrop_filter_label_normal));
        }
    }
    ...
    public void setSelectedFilter(Filter filter) {
        int index = 0;
        for (ThumbnailItem thumbnailItem : thumbnailItemList) {
            if (thumbnailItem.filter == filter) {
                selectedIndex = index;
                notifyDataSetChanged();
                break;
            }
