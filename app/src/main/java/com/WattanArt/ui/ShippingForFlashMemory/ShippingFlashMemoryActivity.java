...
import com.WattanArt.ui.FlashMemory.BitmapFlashHelper;
...
import static com.WattanArt.ui.FlashMemory.FlashMemoryActivity.backFromShipping;
import static com.WattanArt.ui.ShippingForMobile.ShippingMobileActivity.REQUEST_STORAGE_WRITE_PERMISSION;

public class ShippingFlashMemoryActivity extends BaseActivity implements ShippingFlashMemoryMvpView {
    ...
    ImageView bitmap_image_view_front,bitmap_image_view_back;
...
//        getData();
    ...
    void initViews(){
        ...
        if (isNetworkConnected()) {
            ...
            Toast.makeText(ShippingFlashMemoryActivity.this, getString(R.string.error_no_internet_connection), Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void returnBitmapFromHelper(){
        if (BitmapFlashHelper.getInstance().getBitmapFront()!=null){
            saveTempBitmapForFront(BitmapFlashHelper.getInstance().getBitmapFront());
            Log.e("BitmapFlashHelper",BitmapFlashHelper.getInstance().getBitmapFront()+"");
            bitmap_image_view_front.setImageBitmap(BitmapFlashHelper.getInstance().getBitmapFront());
        }
        if (BitmapFlashHelper.getInstance().getBitmapFrontCover()!=null){
            saveTempBitmapForCoverFront(BitmapFlashHelper.getInstance().getBitmapFrontCover());
        }
        if (BitmapFlashHelper.getInstance().getBitmapBack()!=null){
            saveTempBitmapForBack(BitmapFlashHelper.getInstance().getBitmapBack());
        }
        if (BitmapFlashHelper.getInstance().getBitmapBackCover()!=null){
            saveTempBitmapForBackCover(BitmapFlashHelper.getInstance().getBitmapBackCover());
        }
    }
...
//                subCategoryDataAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    ...
    public void returnFromMakeOrder(MobileOrderResponse mobileOrderResponse) {
        ...
        if (BitmapFlashHelper.getInstance()!=null){
            BitmapFlashHelper.getInstance().clearInstance();
...
//                    && validationTool.validatePhone(phoneEditText, getString(R.string.invalid_phone))) {
    ...
    public boolean checkWriteStoragePermission() {
        ...
        int writeStoragePermissionState =
                ContextCompat.checkSelfPermission(ShippingFlashMemoryActivity.this, WRITE_EXTERNAL_STORAGE);
        ...
        if (!writeStoragePermissionGranted) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                ActivityCompat.requestPermissions(ShippingFlashMemoryActivity.this,
    ...
    public void checkCodeAction(){
        if (couponEditText.getText().toString().isEmpty()) {
        ...
        } else if (userData.getUserID(ShippingFlashMemoryActivity.this).isEmpty()) {
            ...
            if (isNetworkConnected()) {
                ...
                couponCodeModel.setUserID(userData.getUserID(ShippingFlashMemoryActivity.this));
