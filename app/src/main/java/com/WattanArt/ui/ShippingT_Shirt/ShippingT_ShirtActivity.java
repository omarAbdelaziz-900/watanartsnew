...
import com.WattanArt.ui.EditDesign.T_ShirtBitmapHelper;
...
public class ShippingT_ShirtActivity extends BaseActivity implements ShippingT_ShirtMvpView {
    ...
    @Inject
    ShippingT_ShirtMvpPresenter<ShippingT_ShirtMvpView> mPresenter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_shipping_t__shirt);
...
//        getData();
    ...
    void initViews(){
        ...
        if (isNetworkConnected()) {
            ...
            Toast.makeText(ShippingT_ShirtActivity.this, getString(R.string.error_no_internet_connection), Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void returnBitmapFromHelper(){
        if (T_ShirtBitmapHelper.getInstance().getBitmapFront()!=null){
            saveTempBitmapForFront(T_ShirtBitmapHelper.getInstance().getBitmapFront());
            Log.e("T_ShirtBitmapHelper",T_ShirtBitmapHelper.getInstance().getBitmapFront()+"");
            bitmap_image_view_front.setImageBitmap(T_ShirtBitmapHelper.getInstance().getBitmapFront());
        }
        if (T_ShirtBitmapHelper.getInstance().getBitmapFrontCover()!=null){
            saveTempBitmapForCoverFront(T_ShirtBitmapHelper.getInstance().getBitmapFrontCover());
        }
        if (T_ShirtBitmapHelper.getInstance().getBitmapBack()!=null){
            saveTempBitmapForBack(T_ShirtBitmapHelper.getInstance().getBitmapBack());
        }
        if (T_ShirtBitmapHelper.getInstance().getBitmapBackCover()!=null){
            saveTempBitmapForBackCover(T_ShirtBitmapHelper.getInstance().getBitmapBackCover());
        }
    }
...
//                subCategoryDataAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    ...
    public void returnFromMakeOrder(MobileOrderResponse mobileOrderResponse) {
        ...
        if (T_ShirtBitmapHelper.getInstance()!=null){
            T_ShirtBitmapHelper.getInstance().clearInstance();
...
//                    && validationTool.validatePhone(phoneEditText, getString(R.string.invalid_phone))) {
    ...
    public boolean checkWriteStoragePermission() {
        ...
        int writeStoragePermissionState =
                ContextCompat.checkSelfPermission(ShippingT_ShirtActivity.this, WRITE_EXTERNAL_STORAGE);
        ...
        if (!writeStoragePermissionGranted) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                ActivityCompat.requestPermissions(ShippingT_ShirtActivity.this,
    ...
    public void checkCodeAction(){
        if (couponEditText.getText().toString().isEmpty()) {
        ...
        } else if (userData.getUserID(ShippingT_ShirtActivity.this).isEmpty()) {
            ...
            if (isNetworkConnected()) {
                ...
                couponCodeModel.setUserID(userData.getUserID(ShippingT_ShirtActivity.this));
